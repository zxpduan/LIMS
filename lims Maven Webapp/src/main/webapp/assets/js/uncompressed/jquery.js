	...
	// Pass this if window is not defined yet
		...
		(typeof window !== "undefined" ? window : this, function(window, noGlobal) {
			...
			(function(window) {
				 ...
				 *          (space-suffixed) string and (if the cache is larger
				 ...
				 * 
				 * @param {Function}
				 *            fn Passed the created div and expects a boolean
				 *            result
				 */
				function assert(fn) {
				 ...
				 * 
				 * @param {Element|Object}
				 *            [doc] An element or document object to use to set
				 *            the document
				...
				setDocument = Sizzle.setDocument = function(node) {
					...
					// Support: IE>8
					// If iframe document is assigned to "document" variable and
					// if iframe has been reloaded,
					// IE will throw "permission denied" error when accessing
					// "document" variable, see jQuery #13936
					// IE6-8 do not support the defaultView property so parent
					// will be undefined
					...
					// Support: IE<8
					// Verify that getAttribute really returns attributes and
					// not properties (excepting IE8 booleans)
					...
					// We allow this because of a bug in IE8/9 that throws an
					// error
				...
				"ATTR" : function(name, operator, check) {
					return function(elem) {
						...
						return operator === "=" ? result === check : operator === "!=" ? result !== check : operator === "^=" ? check && result.indexOf(check) === 0 : operator === "*=" ? check && result.indexOf(check) > -1 : operator === "$=" ? check && result.slice(-check.length) === check : operator === "~=" ? (" " + result + " ").indexOf(check) > -1 : operator === "|=" ? result === check || result
								.slice(0, check.length + 1) === check + "-" : false;
					};
				},
				...
				// Support: Webkit<537.32 - Safari 6.0.3/Chrome 25 (fixed in
				// Chrome 27)
			...
			function Data() {
				// Support: Android < 4,
				// Old WebKit does not have Object.preventExtensions/freeze
				// method,
			...
			set : function(owner, data, value) {
				...
				// and set the unlock as though an owner entry had always
				// existed
			 ...
			 * branch 2. Improve the module's maintainability by reducing the
			 * storage paths to a single mechanism. 3. Use the same single
			 * mechanism to support "private" and "user" data. 4. _Never_ expose
			 * "private" data to user code (TODO: Drop _data, _removeData) 5.
			 * Avoid exposing implementation details on user objects (eg.
			 * expando properties) 6. Provide a clear path for implementation
			...
			remove : function(elem, types, handler, selector, mappedTypes) {
				...
				while (t--) {
					...
					// Unbind all events (on this namespace, if provided) for
					// the element
					if (!type) {
						for (type in events) {
					...
					// Remove generic event handler if we removed something and
			...
			trigger : function(event, data, elem, onlyHandlers) {
				...
				if (!onlyHandlers && !special.noBubble && !jQuery.isWindow(elem)) {
					...
					// Only add window if we got to document (e.g., not plain
					// obj or detached DOM)
			...
			dispatch : function(event) {
				...
				while ((matched = handlerQueue[i++]) && !event.isPropagationStopped()) {
					...
					while ((handleObj = matched.handlers[j++]) && !event.isImmediatePropagationStopped()) {
						...
						// 2) have namespace(s) a subset or equal to those in
						// the bound event (both can have no namespace).
			...
			function fixInput(src, dest) {
				...
				if (nodeName === "input" && rcheckableType.test(src.type)) {
					...
					// Fails to return the selected option to the default
					// selected state when cloning options
			...
			function addGetHookIf(conditionFn, hookFn) {
				...
				return {
					get : function() {
						if (conditionFn()) {
							// Hook not needed (or it's not possible to use it
			...
			(function() {
				...
				if (window.getComputedStyle) {
					...
					pixelPosition : function() {
						// This test is executed only once but we still do
						// memoizing
			...
			style : function(elem, name, value, extra) {
				...
				if (value !== undefined) {
					...
					if (!hooks || !("set" in hooks) || (value = hooks.set(elem, value, extra)) !== undefined) {
						...
						// Setting style to blank string required to delete
						// "style: x !important;"
						style[name] = "";
						style[name] = value;
					}
			...
			jQuery.each([ "height", "width" ], function(i, name) {
				...
				get : function(elem, computed, extra) {
					if (computed) {
						...
						// invisibly show them
						// however, it must have a current display style that
			...
			Tween.propHooks = {
				...
				get : function(tween) {
					...
					// Empty strings, null, undefined and "auto" are converted
					// to 0.
				...
				for (prop in props) {
					...
					if (rfxtypes.exec(value)) {
						...
						if (value === (hidden ? "hide" : "show")) {
							...
							// If there is dataShow left over from a stopped
							// hide or show and we are going to proceed with
			...
			(function() {
				...
				// Check the default checkbox/radio value ("" on old WebKit;
				// "on" elsewhere)
			...
			toggleClass : function(value, stateVal) {
				...
				return this.each(function() {
					if (type === "string") {
						...
						// the above saved it).
						// Otherwise bring back whatever was previously saved
						// (if anything),
			 ...
			 * (mediates between content-type and expected dataType) - returns
			...
			jQuery.expr.filters.hidden = function(elem) {
				// Support: Opera <= 12.12
				// Opera reports offsetWidths and offsetHeights less than zero
				// on some elements
			...
			// keepScripts (optional): If true, will include scripts passed in
			// the html string
			...
			// rather than make the css module depend on the offset module, we
					...
					jQuery.fn[funcName] = function(margin, value) {
						...
						return access(this, function(elem, type, value) {
							...
							if (jQuery.isWindow(elem)) {
								// As of 5/8/2012 this will yield incorrect
								// results for Mobile Safari, but there
								// isn't a whole lot we can do. See pull request
			...
			// with other
			// files that may use define, but not via a proper concatenation
			// script that
			// understands anonymous AMD modules. A named AMD is safest and most
			// robust
			// way to register. Lowercase jquery is used because AMD module
			// names are
...
// /////////////
// resovle bowser is not support after jquery 1.9
(function(jQuery) {
