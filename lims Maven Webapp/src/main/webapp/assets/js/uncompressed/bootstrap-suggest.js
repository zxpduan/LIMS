...
(function (factory) {
    ...
    function getData(keyword, $input, callback, options) {
        ...
        if (options.url) {
        	//add by kevin
        	if (options.loading){
        		$('#'+options.loading).empty().html('<i class="ace-icon fa fa-spinner fa-spin blue fa-2x"></i>');
        	}
        	//end by kevin
    ...
    function getIClear($input, options) {
        ...
        /*
         * add by zxp,for  contract enter page ,because add button near to suggest input
         */
        /////////add start
        var offset_byzxp=null;
        if ($input.attr("id").indexOf("Borrower")!="-1"){
        	offset_byzxp=$input.next('.input-group-btn').width()+73;
        }else{
        	offset_byzxp=$input.next('.input-group-btn').width()+33;
        }        
        ///////////add end
        return $iClear.css({
            position: 'absolute',
            ...
            //by kevin
            right: options.showBtn ? (offset_byzxp|| 33) + 2 : 12,
            zIndex: 4,
            cursor: 'pointer',
            fontSize: 12
        }).hide();
        //the under line is original
        /*
          return $iClear.css({
                position: 'absolute',
                top: 12,
    ...
    var defaultOptions = {
        ...
        loading:'',						//显示loading的id//add by kevin
